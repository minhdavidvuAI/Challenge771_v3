...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        ...
        self.bn1 = nn.GroupNorm(num_groups=16, num_channels=out_channels)
        self.relu = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3,
                               stride=1, padding=1, bias=False)
        #self.bn2 = nn.BatchNorm2d(out_channels)
        self.bn2 = nn.GroupNorm(num_groups=16, num_channels=out_channels)
...
class ResNet18(nn.Module):
    def __init__(self, num_classes=50, input_channels=1):
        ...
        self.bn1 = nn.GroupNorm(num_groups=16, num_channels=self.in_channels)
        self.relu = nn.ReLU(inplace=True)
